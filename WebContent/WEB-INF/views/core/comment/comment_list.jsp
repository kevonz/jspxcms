...
<div class="c_bar">
  <span class="c_position"><s:message code="comment.management"/> - <s:message code="list"/></span>
...
<form action="list.do" method="get">
    ...
    <legend><s:message code="search"/></legend>
	  <label class="c_label"><s:message code="comment.text"/>: <input type="text" name="search_CONTAIN_text" value="${search_CONTAIN_text[0]}"/></label>
	  <label class="c_label"><input type="submit" value="<s:message code="search"/>"/></label>
  </fieldset>
</form>
<form method="post">
<tags:search_params/>
<div class="ls_bc_opt">
	<div class="ls_btn"><input type="button" value="<s:message code="audit"/>" onclick="return optMulti(this.form,'audit.do');"/></div>
	<div class="ls_btn"><input type="button" value="<s:message code="unaudit"/>" onclick="return optMulti(this.form,'unaudit.do');"/></div>
...
<table id="pagedTable" border="0" cellpadding="0" cellspacing="0" class="ls_tb">
  ...
  <tr class="ls_table_th">
    ...
    <th class="ls_th_sort"><span class="ls_sort" pagesort="creator.id"><s:message code="comment.creator"/></span></th>
    <th class="ls_th_sort"><span class="ls_sort" pagesort="fid"><s:message code="comment.text"/></span></th>
    <th class="ls_th_sort"><span class="ls_sort" pagesort="score"><s:message code="comment.score"/></span></th>
    <th class="ls_th_sort"><span class="ls_sort" pagesort="creationDate"><s:message code="comment.date"/></span></th>
  </tr>
  </thead>
  <tbody>
  ...
  <tr ondblclick="location.href=$('#edit_opt_${bean.id}').attr('href');">
    ...
    <td align="center">
      <a href="audit.do?ids=${bean.id}&${searchstring}" class="ls_opt"<c:if test="${bean.status!=0}"> disabled="disabled"</c:if>><s:message code="audit"/></a>
      <a href="unaudit.do?ids=${bean.id}&${searchstring}" class="ls_opt"<c:if test="${bean.status!=1}"> disabled="disabled"</c:if>><s:message code="unaudit"/></a>
    ...
    <td>
    	<div><c:out value="${fnx:substringx_sis(bean.anchor.title,20,'...')}"/></div>
