...
function openIdsToCookie() {
	...
	if(openIds.length>0) {
		...
		$.cookie('open_ids',openIds,{expires:36500});
	} else {
...
<div style="padding:7px 0 3px 0;text-align:center;">
  <label for="siteFile"><input id="siteFile" type="checkbox" onclick="location.href='left.do?noSelect=true'+(this.checked?'&isSiteFile=true':'')"<c:if test="${isSiteFile}"> checked="checked"</c:if>/><s:message code="webFile.siteFile"/></label>
</div>
<hr/>
