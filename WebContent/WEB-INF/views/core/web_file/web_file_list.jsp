	...
	url += "&d="+new Date()*1;
 	$("<div>",{"title":"<s:message code='webFile.moveTo'/>"}).appendTo(document.body).load(url, function(){
 		$(this).dialog({
 			...
 			buttons: {
 				"OK": function() {
 					var moveToDir = $("input:radio[name='moveTodir']:checked");
 					if(moveToDir.length<=0) {
 						alert("please select dir!");
 						return;
 					}
 					var dest = moveToDir.val();
 					$(this).dialog("close");
 					$("<input>",{
 						"type":"hidden",
 						"name":"dest",
...
<div class="c_bar">
  <span class="c_position"><s:message code="webFile.management"/> - <s:message code="list"/> - ${parentId}</span>
...
<form action="list.do" method="get">
    ...
    <legend><s:message code="search"/></legend>
	  <label class="c_lab"><s:message code="webFile.name"/>: <f:text name="search_name" value="${search_name[0]}" style="width:150px;"/></label>
...
<div class="ls_bc_opt">
	<div class="ls_btn"><label for="unzipUpload"><input type="checkbox" id="unzipUpload" onclick="swfupload.setUploadURL(this.checked?'unzip_upload.do':'upload.do');"/><s:message code="webFile.zipUpload"/></label></div>
	<div class="ls_btn"><span id="spanButtonPlaceHolder"></span></div>
	<div class="ls_btn"><input id="btnCancel" type="button" value="<s:message code="cancel"/>" onclick="swfupload.cancelQueue();" disabled="disabled"/></div>	
	<div class="ls_btn"><input type="button" value="<s:message code="webFile.createDir"/>" onclick="$('#createDirDialog').dialog('open');"/></div>
	<div class="ls_btn"><input type="button" value="<s:message code="webFile.createText"/>" onclick="location.href='create.do?parentId=${fnx:urlEncode(parentId)}&${searchstring}';"/></div>
	<div class="ls_btn"></div>
	<div class="ls_btn"><input type="button" value="<s:message code="webFile.zipDownload"/>" onclick="return optPost(this.form,'zip_download.do');"/></div>
	<div class="ls_btn"><input type="button" value="<s:message code="webFile.zip"/>" onclick="return optPost(this.form,'zip.do');"/></div>
	<div class="ls_btn"><input type="button" value="<s:message code="webFile.move"/>" onclick="return optMove(this.form);"/></div>

	<div class="ls_btn"><input type="button" value="<s:message code="delete"/>" onclick="return optDelete(this.form);"/></div>
	<div class="ls_btn"></div>
	<div class="ls_btn"><input type="button" value="<s:message code="webFile.parentDir"/>" onclick="location.href='list.do?parentId=${fnx:urlEncode(ppId)}&${searchstring}';"<c:if test="${empty ppId}"> disabled="disabled"</c:if>/></div>
	<div style="clear:both"></div>
</div>
...
<table id="pagedTable" border="0" cellpadding="0" cellspacing="0" class="ls_tb">
  ...
  <tr class="ls_table_th">
    ...
    <th class="ls_th_sort"><span class="ls_sort" pagesort="name"><s:message code="webFile.name"/></span></th>
    <th class="ls_th_sort"><span class="ls_sort" pagesort="lastModified"><s:message code="webFile.lastModified"/></span></th>
    <th class="ls_th_sort"><span class="ls_sort" pagesort="type"><s:message code="webFile.type"/></span></th>
    <th class="ls_th_sort"><span class="ls_sort" pagesort="length"><s:message code="webFile.length"/></span></th>
  </tr>
  </thead>
  ...
  <tr>
    ...
    <td align="center">
      ...
      <a id="edit_opt_${status.index}" href="${editUrl}" class="ls_opt" style="display:none;"><s:message code="edit"/></a>
      
      <c:url value="zip_download.do?${searchstring}" var="zipDownloadUrl">
 				<c:param name="ids" value="${bean.id}"/>
 			</c:url>
      <a id="zip_download_opt_${status.index}" href="${zipDownloadUrl}" class="ls_opt"<c:if test="${bean.parent}"> disabled="disabled"</c:if>><s:message code="webFile.zipDownload"/></a>
      
      <c:url value="zip.do?${searchstring}" var="zipUrl">
 				<c:param name="ids" value="${bean.id}"/>
 				<c:param name="parentId" value="${parentId}"/>
 			</c:url>
      <a id="zip_opt_${status.index}" href="${zipUrl}" class="ls_opt"<c:if test="${bean.parent}"> disabled="disabled"</c:if>><s:message code="webFile.zip"/></a>

      <c:url value="unzip.do?${searchstring}" var="unzipUrl">
 				<c:param name="ids" value="${bean.id}"/>
 				<c:param name="parentId" value="${parentId}"/>
 			</c:url>
      <a id="unzip_opt_${status.index}" href="${unzipUrl}" class="ls_opt"<c:if test="${bean.parent||!bean.zip}"> disabled="disabled"</c:if>><s:message code="webFile.unzip"/></a>
      ...
      <a id="open_opt_${status.index}" href="${openUrl}"<c:if test="${!bean.directory}"> target="_blank"</c:if> class="ls_opt"<c:if test="${bean.parent}"> disabled="disabled"</c:if>><s:message code="webFile.open"/></a>

      <a href="javascript:void(0);" onclick="optRename('${bean.id}','<c:out value="${bean.name}"/>');" class="ls_opt"<c:if test="${bean.parent}"> disabled="disabled"</c:if></a><s:message code="webFile.rename"/></a>
 			
 			<c:url value="delete.do?${searchstring}" var="deleteUrl">
 				<c:param name="ids" value="${bean.id}"/>
 				<c:param name="parentId" value="${parentId}"/>
 			</c:url>
      <a href="${deleteUrl}" onclick="return confirmDelete();" class="ls_opt"<c:if test="${bean.parent}"> disabled="disabled"</c:if></a><s:message code="delete"/></a>
     </td>
    <td onclick="location.href='${editUrl}';" style="cursor:pointer;">
    	<div id="beanname${status.index}" class="file_${bean.type}"><span<c:if test="${bean.image}"> imgUrl="${bean.url}"</c:if>><c:out value="${bean.name}"/></span></div>
    </td>
