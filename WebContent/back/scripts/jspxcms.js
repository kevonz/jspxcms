...
Cms.uploadImg = function(action,name,button) {
    ...
    if($("#"+fileId).val()=="") {alert("file not selected!");return;}
    var data={};
    if($("#wm_"+name).val()!="undefined") {
    	data.watermark=$("#wm_"+name).val();
    }
    if($("#s_"+name).val()!="undefined") {
    	data.scale=$("#s_"+name).prop("checked");
    }
    if($("#w_"+name).val()!="undefined") {
    	data.width=$("#w_"+name).val();
    }
    if($("#h_"+name).val()!="undefined") {
...
(function($) {
	$.fn.headSort = function() {
		...
		$(".ls_sort").each(function() {
			if(sort == $(this).attr("pagesort")) {
				$(this).addClass("ls_sort_" + dir);
			}
		});
		$(this).find(".ls_th_sort").each(function() {
			var sortSpan = $(this).find(".ls_sort");
			...
			$(this).hover(function() {
				if(currSort!=sort) {
					sortSpan.addClass("ls_sort_asc");
				}
			},function() {
				if(currSort!=sort) {
					sortSpan.removeClass("ls_sort_asc");
				}
			});
		});
	};
